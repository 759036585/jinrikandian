      ...
      <div class="channel">
        ...
        <van-grid class="van-hairline--left">
          <van-grid-item v-for="(index,i) in channels" :key="index.id">
            ...
            <van-icon class="btn" name="cross" v-show="editing" v-if="i !== 0" @click="$emit('delChannel',index.id)"></van-icon>
          </van-grid-item>
        </van-grid>
      </div>
      <div class="channel">
        ...
        <van-grid class="van-hairline--left">
          <van-grid-item v-for="index in optionalChannels" :key="index.id">
            ...
            <van-icon class="btn" name="plus" @click="$emit('addChannel',index)" v-show="editing"></van-icon>
          </van-grid-item>
        </van-grid>
      </div>
    </div>
